...
const { sequelize, User, Leaderboard, Admin } = require('./models');
// GEMINI AI CONFIG
// ------------------------
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const decryptedUser = newUser.get({ plain: true });
        decryptedUser.phone = decrypt(decryptedUser.phone);
        
        res.json({ token, user: { id: decryptedUser.id, name: decryptedUser.name, email: decryptedUser.email, phone: decryptedUser.phone } });
    } catch (err) {
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const decryptedUser = user.get({ plain: true });
        decryptedUser.phone = decrypt(decryptedUser.phone);
...
app.post('/api/admin/login', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Admin login failed: " + err.message });
    }
});
...
app.get('/api/profile', auth, (req, res) => {
    const userProfile = req.user.get({ plain: true });
...
app.get('/api/admin/students', adminAuth, async (req, res) => {
    try {
        ...
        const decryptedUsers = users.map(u => {
            ...
            plainUser.phone = decrypt(plainUser.phone);
            return plainUser;
        });
...
// ------------------------
// Dynamic Quiz Generation (Gemini)
// ------------------------
app.post('/api/generate-quiz', async (req, res) => {
    try {
        const { topic, count } = req.body;
        const numQuestions = parseInt(count) || 10;
        const quizTopic = topic || 'General Tech';
        ...
        const prompt = `
            You are an expert technical interviewer. Generate a multiple-choice quiz about ${quizTopic}.
            The quiz must contain exactly ${numQuestions} questions.
            The difficulty should be appropriate for a B.Tech student preparing for MAANG interviews.

            Return ONLY a valid JSON array of objects with the following strict structure:
            [
                {
                    "q": "The question text here?",
                    "options": ["Option 1", "Option 2", "Option 3", "Option 4"],
                    "a": 0 // Integer representing the 0-indexed correct option
                }
            ]

            DO NOT include markdown formatting (\`\`\`json), explanations, or any other text outside the JSON array.
        `;

        console.log(`[Quiz] Generating ${numQuestions} questions for topic: ${quizTopic}...`);
        const responseText = await getGeminiResponse(prompt);
        ...
        try {
            ...
            if (!Array.isArray(questions)) {
             ...
             console.error("[Quiz] JSON Parse Error. Raw output:", responseText);
             res.status(500).json({ error: "Failed to generate quiz. The AI returned an invalid format. Please try again." });
        }
...
app.get('/api/live-internships', (req, res) => {
    ...
    const data = [
        ...
        {
            title: "SDE-1 (Fixed Term Contract)",
            company: "AWS",
            location: "India",
            stipend: "High Paying",
            track: "Software Development",
            apply_url: "https://www.amazon.jobs/en/jobs/3170280/sde-1-fixed-term-contract"
        },
        {
            title: "AI Research Internship (2025-27 Batch)",
            company: "Lexsi.ai",
            location: "India",
            stipend: "Stipend Available",
            track: "Data / AI",
            apply_url: "https://app.screenloop.com/careers/aryaxai/job_posts/6659?tab=overview"
        },
        {
            title: "Software Engineer (2023-25 Batch)",
            company: "Hewlett Packard Enterprise",
            location: "India",
            stipend: "Competitive",
            track: "Software Development",
            apply_url: "https://careers.hpe.com/us/en/job/1197623/Software-Engineer"
        },
        {
            title: "MTS - 2 (2022-24 Batch)",
            company: "Adobe",
            location: "Remote/India",
            stipend: "High Paying",
            track: "Software Development",
            apply_url: "https://careers.adobe.com/us/en/job/ADOBUSR165046EXTERNALENUS/MTS-II"
        },
        {
            title: "SDE-1 (2024-25 Batch)",
            company: "Microsoft",
            location: "India",
            stipend: "High Paying",
            track: "Software Development",
            apply_url: "https://apply.careers.microsoft.com/careers/job/1970393556633063?domain=microsoft.com&hl=en"
        },
        {
            title: "SDE-1 (2023-25 Batch)",
            company: "Amazon Pay",
            location: "India",
            stipend: "High Paying",
            track: "Software Development",
            apply_url: "https://www.amazon.jobs/en/jobs/3176671/software-dev-engineer-i"
        },
        {
